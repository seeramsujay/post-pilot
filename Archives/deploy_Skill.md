...
## When to use
You need to compile the Vite React frontend and Node.js backend into a unified Devvit bundle on the remote PC and upload it to Reddit.

## Prerequisites
- SSH connection to the remote PC (RTX 4050) is active
- Devvit CLI installed globally (`npm install -g @devvit/cli`)
- Authenticated with Reddit via `devvit login` on the remote machine
...
3. Run `devvit install <subreddit-name>` to deploy the uploaded version to your test subreddit.
